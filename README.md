...
- Place BrainVision `.vhdr/.vmrk/.eeg` files in a BIDS-like layout. By default the code expects `ds005620/sub-XXXX/eeg/*.vhdr`, but any path can be provided via CLI flags.

## Quickstarts
- Single file:  
  `python random_channels_mib.py --vhdr ds005620/sub-1010/eeg/sub-1010_task-awake_acq-EO_eeg.vhdr --mode both --estimators ksg --n-channels 8 --repeats 20 --epoch-lengths 5 --output results/mib_random_channels`
- Dataset sweep (uses `create_subject_file_map` to pick supported conditions):  
  `python random_channels_mib.py --dataset ds005620 --subjects sub-1010 sub-1011 --mode broadband --estimators ksg binning --epoch-lengths 5 10 --n-channels 8 --repeats 30`
...
- Summaries: `analyze_mib_results.py` reads the JSON outputs and reports per-condition stability plus cross-subject differences. Example: `python analyze_mib_results.py --root results/mib_random_channels --mode broadband --estimator ksg`.
