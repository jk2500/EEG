...
ROOT = Path("results/ds005620/mib_random_channels/spectral/binning/epoch-10p00s")
OUT_DIR = Path("analysis/outputs/ds005620/all_subjects_results_epoch10")
...
def main() -> None:
    ...
    df.to_csv(OUT_DIR / "epoch10_band_means.csv", index=False)
    ...
    wide.to_csv(OUT_DIR / "epoch10_composites.csv", index=False)
    ...
    comp_df.to_csv(OUT_DIR / "epoch10_awake_vs_sedation.csv", index=False)
    ...
    plt.title("Epoch 10s: Awake_avg vs Sedation by Band (95% CI)")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "epoch10_awake_avg_vs_sedation_by_band.png", dpi=150)
    plt.close()
    ...
    plt.title("Epoch 10s: Awake vs Sedation by Band")
    plt.legend()
    plt.tight_layout()
    plt.savefig(OUT_DIR / "epoch10_awake_vs_sedation_by_band.png", dpi=150)
    plt.close()
    ...
    plt.ylabel("Awake_avg - Sedation (mean diff)")
    plt.title("Epoch 10s: Composite Metrics (Awake_avg vs Sedation)")
    plt.tight_layout()
    plt.savefig(OUT_DIR / "epoch10_awake_avg_vs_sedation_composites.png", dpi=150)
    plt.close()
