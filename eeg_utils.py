...
def generate_bipartitions(n_channels, max_partitions=None, verbose=True):
    ...
    if max_partitions and max_partitions < total_partitions:
        log_print(f"Limiting to {max_partitions} random partitions from {total_partitions}.", verbose)
        indices = np.random.choice(total_partitions, max_partitions, replace=False)
        return [all_partitions[i] for i in indices]
    return all_partitions

def generate_all_subsets(n_channels, min_size=2, max_size=None):
    """
    Generate all possible subsets of channels (power set) for IIT complexes calculation.
    ...
    min_size : int, default=2
        Minimum subset size (need at least 2 channels for bipartition)
    max_size : int, optional
        Maximum subset size (default: n_channels)
        
    Returns:
    --------
    all_subsets : list
        List of tuples, each representing a subset of channel indices
    """
    if max_size is None:
        max_size = n_channels
        
    all_subsets = []
    for subset_size in range(min_size, min(max_size + 1, n_channels + 1)):
        for subset in combinations(range(n_channels), subset_size):
            all_subsets.append(subset)
    return all_subsets

def find_local_maxima(phi_values, top_n=30):
    """
    Find subsets whose Φ is a local maximum (IIT complexes).
    
    A subset S is a complex if no superset has higher Φ.
    
    Parameters:
    -----------
    phi_values : dict
        Dictionary mapping subset tuples to their Φ values
    top_n : int, default=30
        Number of top complexes to return
        
    Returns:
    --------
    complexes : list
        List of (subset, phi_value) tuples sorted by Φ value (descending)
    """
    complexes = []
    
    for subset, phi in phi_values.items():
        is_local_max = True
        
        # Check all supersets of this subset
        for other_subset, other_phi in phi_values.items():
            if (set(subset).issubset(set(other_subset)) and 
                subset != other_subset and 
                other_phi > phi):
                is_local_max = False
                break
        
        if is_local_max:
            complexes.append((subset, phi))
    
    # Sort by Φ value (descending), return top N
    complexes.sort(key=lambda x: x[1], reverse=True)
    return complexes[:top_n]
...
def analyze_spectral_complexity(file_paths, complexity_func, n_channels, verbose, n_jobs, **kwargs):
    """
    Run complexity analysis across different spectral bands for multiple files.
    """
    spectral_results = {}
    for condition, file_path in file_paths.items():
        log_print(f"Analyzing spectral bands for {condition}...", verbose)
        raw = mne.io.read_raw_brainvision(file_path, preload=True, verbose=False)
        
        band_epochs, _ = preprocess_eeg_by_bands(raw, n_channels=n_channels, **kwargs)
        
        condition_results = {}
        for band, epochs_data in band_epochs.items():
            log_print(f"  Processing band: {band}", verbose)
            
            with Parallel(n_jobs=n_jobs) as parallel:
                metric_values = parallel(
                    delayed(complexity_func)(epoch) for epoch in tqdm(epochs_data, desc=f"Band {band}", disable=not verbose)
                )
            
            metric_values = [v for v in metric_values if v is not None]
            if metric_values:
                condition_results[band] = {
                    'mean_metric': np.mean(metric_values),
                    'std_metric': np.std(metric_values)
                }
        spectral_results[condition] = condition_results
