...
\begin{itemize}[leftmargin=*]
    \item \textbf{Primary dataset: ds005620 (BrainVision EEG-BIDS).}\newline A repeated-awakening propofol-sedation study distributed in EEG-BIDS/BrainVision format. Repository path listed in Table~\ref{tab:path_index}. We use resting-state EEG recordings in three conditions:
    \begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Robustness dataset: Sedation-RestingState (EEGLAB).}\newline An independent resting-state sedation dataset distributed as EEGLAB files (\texttt{.set}/\texttt{.fdt}) with index file \texttt{datainfo.mat}. Repository path listed in Table~\ref{tab:path_index}. Recordings are already epoched ($\approx$\SI{10}{s}), with 91 channels and a native sampling rate of \SI{250}{Hz}. We analyze four within-subject conditions: baseline, light sedation, deep sedation, and recovery ($N=20$ subjects with all four conditions).
\end{itemize}
...
\section{Results}
We treat the BrainVision EEG-BIDS dataset ds005620 as the \emph{primary} dataset and structure the results to reflect the experimental logic: (i) choose an estimator and hyperparameters using diagnostic tests on ds005620, (ii) run the fixed analysis on the full ds005620 cohort to identify the most discriminative representation, and (iii) evaluate robustness by applying the same pipeline to an independent sedation dataset.

\subsection{Primary dataset: estimator diagnostics and hyperparameter selection (ds005620)}
\label{sec:ds005620_calibration}
We first select an estimator and stable operating point. To avoid \emph{tuning on} across-subject effects, diagnostics and sweeps are performed on a representative ds005620 subject (sub-1067) across awake eyes-open, awake eyes-closed, and sedation.

\subsubsection{Gaussianity diagnostics motivate nonparametric estimation}
We explicitly tested whether EEG epochs are well-approximated as Gaussian. Q--Q plots show systematic deviations from normality with heavy tails, and normality tests reject the Gaussian hypothesis at conventional thresholds. This motivates using a nonparametric estimator (binning) as the primary estimator and using the Gaussian estimator only as a diagnostic baseline (Eq.~\eqref{eq:gaussian_mi}).

\input{robustness_gaussianity.tex}
...
\subsubsection{Epoch-length and channel-count trade-offs}
We also probed sensitivity to epoch length and number of channels on representative ds005620 recordings (sub-1067). Two robust patterns emerge:
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Channel count:} mean MIB increases with channel count while CV generally decreases, but condition differences can shift with $k$ because both the estimator and the partition sampling interact with dimensionality.
\end{itemize}
...
\paragraph{Chosen operating point.}
Based on these diagnostics (and computational constraints from bipartition growth and integer-state encoding), we fix the binning estimator with $B=10$, analyze $k=16$ channels per repeat with 50 random repeats, and use \SI{5}{s} epochs as the primary window length for ds005620. This defines the operating point for the main ds005620 experiment below; longer epochs (\SI{10}{s}) are reported as a sensitivity analysis.

\subsection{Primary dataset main experiment: across-subject broadband and spectral MIB (ds005620)}
\label{sec:ds005620_main_results}
Using the fixed operating point above, we compute MIB in canonical spectral bands (delta, theta, alpha, beta, gamma) as well as a broadband analysis. All across-subject summaries use $N=20$ subjects with \texttt{sedation\_1} and both awake conditions available.

\paragraph{Condition means by band.}
\input{ds005620_condition_means_epoch5.tex}

\paragraph{State discrimination and alpha-band selection.}
The key empirical constraint is that eyes-open and eyes-closed are not interchangeable: some bands separate eyes-closed from sedation strongly, while eyes-open shows mixed behavior (including \emph{awake $<$ sedation} effects). Full statistics for all bands and composite metrics are reported in Supplementary Table~\ref{tab:ds005620_all_subjects_epoch5}; selected highlights are:
\begin{itemize}[leftmargin=*]
...
\end{itemize}
Taken together, these results nominate alpha-band MIB as the most consistent \emph{single-band} discriminator of wakefulness versus sedation on ds005620 when aggregating wakeful subconditions, while also showing that no single band yields a uniformly positive awake-versus-sedation signature across both awake eyes-open and awake eyes-closed.
...
\subsection{Sensitivity analyses on the primary dataset (ds005620)}
\subsubsection{Epoch-length sensitivity (\SI{10}{s} epochs)}
...
\input{ds005620_condition_means_epoch10.tex}
...
\subsubsection{Within-subject illustration (sub-1010)}
To make the estimator behavior concrete at the level of a single recording, we report an illustrative within-subject ds005620 analysis (sub-1010), which exposes the scale of repeat-to-repeat variability under random channel sampling and the qualitative differences between eyes-open, eyes-closed, and sedation within one subject.
...
\paragraph{Condition means by band.}
\input{ds005620_sub1010_condition_means.tex}
...
\subsection{Robustness on an independent dataset: Sedation-RestingState}
\label{sec:sedation_robustness}
To test whether the primary-dataset conclusions (including the prominence of alpha-band effects) are robust to dataset differences, we apply the same pipeline to an independent Sedation-RestingState dataset spanning baseline, light sedation, deep sedation, and recovery. We use the same binning estimator and channel-sampling design (50 repeats, $k=16$) and analyze the native $\approx$\SI{10}{s} epochs provided by the dataset.

\paragraph{Across subjects, sedation depth effects are small and non-monotonic.}
Across $N=20$ subjects, band-limited MIB varies across the four conditions, but changes are modest and do not follow a consistent monotone trajectory from baseline $\rightarrow$ light $\rightarrow$ deep $\rightarrow$ recovery. In particular, light sedation often increases MIB relative to baseline across multiple bands, while deep sedation decreases relative to light sedation; recovery frequently returns toward baseline but not uniformly.

\paragraph{Condition-level means and variability.}
\input{sedation_fourway_band_summary.tex}
...
Paper regeneration pipeline & \path{run_optimal_analysis.sh} \\
Entry script (MIB computation) & \path{scripts/mib_analysis.py} \\
Core analysis package & \path{src/eeg_analysis/} \\
Generated results (ds005620, \SI{5}{s}) & \path{results/ds005620/mib_analysis_optimal/}\newline\path{ds005620/spectral/binning/} \\
Generated results (ds005620, \SI{10}{s}) & \path{results/ds005620/mib_analysis_optimal_epoch10/}\newline\path{ds005620/spectral/binning/} \\
Generated results (sedation) & \path{results/sedation_resting_state/mib_sedation/}\newline\path{sedation/spectral/binning/} \\
...
\input{ds005620_sub1010_condition_comparisons.tex}
\input{ds005620_all_subjects_awake_vs_sedation_epoch5.tex}
\input{ds005620_all_subjects_awake_vs_sedation_epoch10.tex}
\input{sedation_fourway_pairwise.tex}
